...
const SlideVideoSection = React.forwardRef(
  (
    ...
    useEffect(() => {
      // We only want to handle this if NOT using video.js player instance
      // or if we need to update the player instance src
      if (slideVideoRef.current && hasSlideInitialized && videos?.length > 0) {
        ...
        if (newSrc && newSrc !== lastSlideSrc) {
          ...
          // If player instance exists, use it
          if (playerRef.current) {
            playerRef.current.src({ src: newSrc, type: getVideoType(newSrc) });
            if (answerPptIndex !== null) {
            ...
            // Fallback for native (though unexpected if init works)
            if (!slideVideo.paused) {
              slideVideo.pause();
            }
            slideVideo.src = newSrc;
            slideVideo.load();
          }
    ...
    }, [currentVideoIndex, videos, preloadedSlideIndex, hasSlideInitialized, answerPptIndex, lastSlideSrc]);

    // Initialize Video.js on the slide video ref
    const playerRef = useRef(null);

    useEffect(() => {
      // Delay initialization slightly to ensure DOM is ready
      const timer = setTimeout(() => {
        if (slideVideoRef.current && !playerRef.current && !isOnlyVideoMode) {
          console.log("Initializing video.js for slide video");

          // Cleanup existing player if any (though logic prevents this usually)
          if (slideVideoRef.current.player) {
            slideVideoRef.current.player.dispose();
          }

          playerRef.current = videojs(slideVideoRef.current, {
            controls: false,
            muted: true,
            autoplay: false,
            playsinline: true,
            ...
            html5: {
              vhs: {
                overrideNative: true, // Force VHS usage on Safari for headers
                xhr: {
          ...
          if (videos?.[currentVideoIndex]?.slide_video) {
            const src = videos[currentVideoIndex].slide_video;
            playerRef.current.src({ src, type: getVideoType(src) });
          }

          // Ensure we handle disposal
    ...
    const videoIndex = answerPptIndex !== null ? answerPptIndex : currentVideoIndex;
    const slideImage =
      slideNumbers.length > 0 ? videos?.find((video) => video.slide === slideNumbers[0])?.slide_image_url : null;
    ...
    return (
      <div className="w-full h-full bg-black rounded-xl overflow-hidden relative">
        ...
        <div data-vjs-player className="w-full h-full">
          <video
            ref={slideVideoRef}
            className="video-js vjs-default-skin w-full h-full object-fill"
            playsInline
            muted
            onLoadStart={() => {
              if (!isVideoLoading) {
            ...
            onCanPlay={() => {
              if (!canPlay) {
                setCanPlay(true);
                // We can trust the ref or player if initialized
                const player = playerRef.current || slideVideoRef.current;
                if (player) {
                  // If using video.js, use API
                  const setCurrentTime = (t) => {
                    if (player.currentTime && typeof player.currentTime === "function") player.currentTime(t);
